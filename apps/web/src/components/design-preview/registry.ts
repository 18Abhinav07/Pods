...
const labels: Partial<Record<ScreenId, [string, string]>> = {
  ...
  "signature-waiting": [
    "Signature Waiting",
    "The wallet handoff remains visible until Nimiq Pay responds."
  ],
  "signature-error": [
    "Signature error",
    "A recoverable wallet-signature failure."
  ],
  discover: ["Discover", "Browse public activity communities."],
  "public-pod-details": [
    "Pod preview",
    "Understand the group before applying."
  ],
  "visitor-room": ["Visitor room", "Read-only public activity."],
  "public-proof": ["Public proof", "Only explicitly shared evidence."],
  application: ["Application", "Apply without implying a reserved place."],
  today: ["Today", "The single most important action."],
  "my-pods": ["My Pods", "Every Pod and its next real action."],
  "funding-summary": ["Funding", "The current financial state."],
  "funding-waiting": ["Waiting room", "Cutoff, roster, and schedule."],
  "pod-room": ["Pod Room", "Chat-first group activity."],
  ...
  "proof-approved": ["Proof Approved", "A concise earned outcome."],
  "refund-reason": ["Refund", "Amount, reason, state, and next action."],
  "settlement-calculated": [
    "Settlement",
    "Payout first, detail on demand."
  ],
  ...
  "profile-privacy": ["Profile Privacy", "Visibility and contact boundaries."],
...
export const ACTOR_DEFINITIONS: Record<PreviewActorId, ActorDefinition> = {
  ...
  creator: {
    ...
    initialScreen: "creator-command-center",
    screens: [
      "creator-command-center",
      "applications",
      "application-detail",
...
export const TRANSITION_REGISTRY: readonly TransitionDefinition[] = [
  ...
  transition("discover", "open-pod", "public-pod-details", ["visitor", "participant"]),
  transition("public-pod-details", "watch", "visitor-room", ["visitor", "participant"]),
...
export function dispatchPreviewAction(
  ...
  switch (action.type) {
    ...
    case "open-screen": {
      const actor = ACTOR_DEFINITIONS[state.actor];
      if (!actor.screens.includes(action.screen)) {
        throw new Error(
          `${state.actor} cannot open ${action.screen}; the screen is outside this actor journey.`
        );
      }
