...
All 60 decisions below were approved on 2026-07-28 and now define the
canonical Mainnet object and relationship graph.

### Canonical object model

1. **Which first-class durable objects exist?**

   Locked decision: the initial canonical set is Person, Organization, Team,
...
2. **How is a community, club, company, or program represented?**
   ...
   Locked decision: use one Organization object with an explicit organization
...
3. **What is a Team?**
   ...
   Locked decision: a Team is a durable working group of people that may work
   across Projects, Events, and Pods. It is not recreated for every Event.

4. **What is an Event?**

   Locked decision: an Event is a time-bounded program, season, campaign,
   challenge, competition, or gathering with its own participation and
   completion boundary. It is not a chatroom or an identity.

5. **What is a Project?**

   Locked decision: a Project is a durable outcome or body of work that can
   exist independently, span Events, involve changing contributors, and retain
   one continuous history.

6. **What is a Pod?**

   Locked decision: a Pod is a focused execution and accountability structure
...
10. **How are object identities represented?**
    ...
    Locked decision: every object receives an opaque immutable ID. Names,
...
11. **How does an Organization relate to an Event?**
    ...
    Locked decision: use an explicit `OrganizationHostsEvent` relationship with
...
12. **How does an Organization relate to a Team or Project?**
    ...
    Locked decision: use explicit affiliation or ownership relationships.
...
13. **How does a Team relate to a Project?**
    ...
    Locked decision: use a typed `TeamWorksOnProject` relationship. A Project
    may involve several Teams, and a Team may work across several Projects.

14. **Must a Project have a Team?**

    Locked decision: no. A Project may be owned and developed by one person,
    direct contributors, one Team, or several Teams.

15. **Does Team membership make someone a Project contributor?**

    Locked decision: no. Project contribution is an explicit relationship.
...
16. **May a person contribute to a Project without joining its Team?**
    ...
    Locked decision: yes. Mentors, specialists, reviewers, and short-term
...
17. **What happens to a Team or Project after an Event ends?**
    ...
    Locked decision: both persist independently. Event-specific status and
...
18. **Can one Project enter multiple Events?**
    ...
    Locked decision: yes. Each entry is a separate EventEntry with its own
...
19. **What represents participation in an Event?**
    ...
    Locked decision: use a first-class EventEntry rather than placing Event
...
20. **What may an EventEntry represent?**
    ...
    Locked decision: its declared entry subject is exactly one Person, Team, or
...
21. **How are the people in an EventEntry represented?**
    ...
    Locked decision: use EventParticipant records linked to the entry. The
...
22. **Can one person appear twice in the same EventEntry through several
    ...
    Locked decision: no. Enforce one active EventParticipant per Person and
...
23. **May one person participate in several entries in the same Event?**
    ...
    Locked decision: the graph permits it, but each Event contract must
...
24. **Does registering or applying create an EventEntry?**
    ...
    Locked decision: no. Applications and invitations are intents. An
...
25. **Where do Event-specific submission and category details live?**
    ...
    Locked decision: store them on EventEntry or its typed Event records, never
    on the durable Project, Team, or Person.

26. **How is the Event roster frozen?**

    Locked decision: create an immutable roster snapshot when the Event contract
...
27. **How does a Pod attach to the broader graph?**
    ...
    Locked decision: every Pod declares exactly one primary host context:
    standalone, Organization, Team, Event, Project, or EventEntry.

28. **Can a Pod have several primary parents?**

    Locked decision: no. Additional display paths are derived from typed
...
29. **Can a Pod change its primary host after activation?**
    ...
    Locked decision: no. A draft may change host, but an activated Pod keeps the
    frozen host context. Moving it creates a new Pod or an explicitly linked
    successor.

30. **What represents a person's place in a Pod?**

    Locked decision: use a PodParticipation record. It owns participation state,
...
31. **Does broader membership automatically create Pod participation?**
    ...
    Locked decision: no. Organization, Team, Project, and Event relationships
...
35. **Where is the term membership used?**
    ...
    Locked decision: reserve Membership for durable belonging to an
...
36. **What is a ProjectContribution?**
    ...
    Locked decision: it is a Person-to-Project relationship with a declared
...
37. **Can one person have several contribution roles in a Project?**
    ...
    Locked decision: yes, through separate compatible role grants attached to
...
38. **How many active Membership records may exist per person and target?**
    ...
    Locked decision: one. Role changes update or add role grants around that
...
39. **Are followers, friends, guardians, sponsors, reviewers, beneficiaries,
    ...
    Locked decision: no. Each is a separate relationship or contextual role
...
40. **Can a service or integration actor become a member or participant?**
    ...
    Locked decision: no. Services may observe or submit attributed facts through
...
41. **What happens to authorship when a person leaves?**
    ...
    Locked decision: all historical messages, artifacts, proof, decisions, and
...
42. **Can a person belong to many Organizations, Teams, Projects, Events, and
    ...
    Locked decision: yes. Pods imposes no global exclusivity. Constraints are
...
43. **What are applications and invitations?**
    ...
    Locked decision: they are separate, expiring intent records that request a
...
44. **How is an invitation to an unregistered person represented?**
    ...
    Locked decision: issue a revocable opaque invitation token. Bind it to the
...
45. **Can applications or invitations be duplicated?**
    ...
    Locked decision: permit only one open intent per person, target, and
...
46. **What happens when an application is accepted but later requirements
    ...
    Locked decision: move it to `accepted_pending_requirements`. Do not label
...
47. **Does acceptance reserve capacity?**
    ...
    Locked decision: not by default. Capacity is consumed by the target's
...
48. **How are imported Team rosters handled?**
    ...
    Locked decision: an organizer may propose invitations in bulk, but every
...
49. **Where is capacity defined?**
    ...
    Locked decision: capacity belongs to the exact target contract, such as an
...
50. **When is a financial Pod seat secured?**
    ...
    Locked decision: only after acceptance, required consent, valid finalized
...
51. **How is oversubscription represented?**
    ...
    Locked decision: preserve every application, acceptance, and funding fact,
...
52. **May an active person leave immediately?**
    ...
    Locked decision: yes when no frozen obligation remains. Otherwise the
...
53. **What happens when a person is removed?**
    ...
    Locked decision: disable future actions and record the reason and actor.
...
54. **Can ownership or required leadership silently disappear when someone
    ...
    Locked decision: no. Transfer required authority before exit or place the
    object in an explicit blocked governance state with only policy-approved
    recovery actions.

55. **How do youth participate in this graph?**

    Locked decision: eligible youth may hold non-financial Membership,
...
56. **Who can own an object?**
    ...
    Locked decision: exactly one current owner is either a canonical Person or
...
57. **How does ownership transfer work?**
    ...
    Locked decision: transfer is an explicit offer, acceptance, and effective
...
58. **When may an object be deleted instead of archived?**
    ...
    Locked decision: only an unactivated draft with no accepted relationship,
...
59. **Does archival cascade through the graph?**
    ...
    Locked decision: no. Archiving an Organization, Team, Event, Project, or
...
60. **How are graph changes made auditable and safe?**
    ...
    Locked decision: create, link, unlink, activate, allocate, lock, transfer,
...
## Locked Relationship Vocabulary
...
## Locked Intent and Relationship States
...
All 60 object, relationship, ownership, membership, enrollment, capacity,
exit, and historical-continuity decisions are resolved. The resulting graph
contains no recursive ownership, implied participation, duplicated identity,
or ambiguous generic edge.
