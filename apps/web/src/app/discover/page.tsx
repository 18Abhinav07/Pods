...
export default async function DiscoverPage({
  ...
  return (
    <main className={`app-shell adaptive-discover ${styles.shell}`}>
      ...
      <section className={styles.intro}>
        <p className={styles.routeLabel}>Find your next rhythm</p>
        <p>Apply to open groups or watch visitor-enabled Pods build in public.</p>
      </section>
      <nav className={styles.stageSelector} aria-label="Pod lifecycle">
      ...
      <div className={styles.filterScroller}>
        <nav className={styles.templateFilters} aria-label="Filter public Pods">
      ...
      {cards.length > 0 ? (
        <section className={`public-pod-grid ${styles.podList}`} aria-label="Public Pods">
        ...
        <section className={styles.emptyState}>
          ...
          {activeTemplate ? <Link className={styles.secondaryAction} href={`/discover?stage=${activeStage}`}>Clear filter</Link> : null}
          <Link className={styles.primaryAction} href="/pods/create/template">Create a Pod</Link>
        </section>
      )}
