...
export default async function RulesPage({ params }: { params: Promise<{ podId: string }> }) {
  ...
  return <main className={styles.shell}>
    <header className={styles.simpleHeader}><Link href="/today"><span className="pod-mark" aria-hidden="true" />pods</Link><span className={styles.frozenPill}>Contract frozen</span></header>
    <section className={styles.intro}><span>Immutable Pod rules</span><h1>{contract.activity.name}</h1><p>{contract.activity.purpose}</p></section>
    <div className={styles.contractHash}><span>Contract fingerprint</span><code>{pod.contractHash}</code></div>
    <div className={styles.ruleGrid}>
      <section className={styles.ruleCard}><span>Template</span><strong>{template?.name}</strong><p>{template?.evidence}</p></section>
      <section className={styles.ruleCard}><span>Schedule</span><strong>{contract.commitment.occurrenceCount} frozen occurrences</strong><p>{contract.activity.startDate} to {contract.activity.endDate}, {contract.activity.timeZone}</p></section>
      <section className={styles.ruleCard}><span>Community</span><strong>{contract.community.visibility === "public" ? "Public application community" : "Private invitation community"}</strong><p>{contract.community.minParticipants} minimum, {contract.community.maxParticipants} maximum</p></section>
    </div>
    <details className={styles.ruleDisclosure} open>
      <summary>Financial and verification terms</summary>
      <div>
        <section className={styles.ruleCard}><span>Commitment</span><strong>{nim(contract.commitment.totalLuna)} Testnet NIM upfront</strong><p>{contract.settlementMode === "full_refund_alpha" ? "Full return after roster lock. This contract cannot switch to proportional redistribution." : `${nim(contract.commitment.lunaPerOccurrence)} NIM per occurrence`}</p></section>
        {verifierAuthority?.source === "testnet_override" ? (
          <>
            <section className={styles.ruleCard}><span>Frozen evidence authority</span><strong>Pods Team review</strong><p>The frozen contract fingerprint remains unchanged.</p></section>
            <section className={styles.ruleCard}><span>Testnet operational amendment</span><strong>Creator review active</strong><p>The Pod creator now reviews member proofs. The creator remains outside membership and financial records. This audited amendment does not change the frozen contract hash or return policy.</p></section>
          </>
        ) : (
          <section className={styles.ruleCard}><span>Evidence authority</span><strong>{effectiveVerifier === "creator" ? "Creator review" : "Pods Team review"}</strong><p>{effectiveVerifier === "creator" ? contract.settlementMode === "full_refund_alpha" ? "The Pod creator reviews member proofs. The creator does not fund this Pod or receive any member funds." : "The Pod creator reviews member proofs. Approval and rejection can change how member stakes are redistributed. The creator does not fund this Pod or receive member funds. This Testnet MVP has no appeal or peer vote. Fund only if you trust the creator and accept these frozen rules." : "The Pods Team reviews member proofs under this legacy frozen contract."}</p></section>
        )}
        <section className={styles.ruleCard}><span>{contract.settlementMode === "full_refund_alpha" ? "Return policy" : "Timeout protection"}</span><strong>{contract.settlementMode === "full_refund_alpha" ? "100% full-return alpha" : "24-hour hard protection"}</strong><p>{contract.settlementMode === "full_refund_alpha" ? "Activity review affects the participant record, never the return amount." : "Principal protected, no bonus, streak extended."}</p></section>
      </div>
    </details>
    <Link className={styles.primaryLink} href={nextHref}>{nextLabel}</Link>
  </main>;
}
