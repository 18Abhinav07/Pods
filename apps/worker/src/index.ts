...
export async function readDepositWorkerConfiguration(
  ...
  return { network, treasuryAddress, rpcUrl, databaseUrl, pollIntervalMs } as const;
}

export async function startDepositWorker() {
  const configuration = await readDepositWorkerConfiguration();
  const repository = createPodsRepository(configuration.databaseUrl);
  const rpc = new NimiqDepositRpc(configuration.rpcUrl);
  let stopping = false;
...
if (import.meta.url === entryPath) {
  startDepositWorker().catch((error: unknown) => {
    console.error(error instanceof Error ? error.message : "Deposit worker failed to start");
    process.exitCode = 1;
  });
}
